...
class ManualBotManagementViewController: NSViewController {
    ...
    private var buildTemplates: [BuildTemplate] {
        return self.storageManager.buildTemplates.values.sorted { $0.id < $1.id }
    }
    ...
    override func viewWillAppear() {
        ...
        self.fetchBranches { (_, error) -> Void in
    ...
    func fetchBranches(_ completion: @escaping ([BranchType]?, Error?) -> Void) {

        self.branchActivityIndicator.startAnimation(nil)
        let repoName = self.syncer.project.serviceRepoName()!
        self.syncer.sourceServer.getBranchesOfRepo(repo: repoName, completion: { (branches, error) -> Void in
    ...
    func createBot() {
            ...
            let template = self.pullTemplate()
        {
            ...
            XcodeServerSyncerUtils.createBotFromBuildTemplate(botName: name, syncer: syncer, template: template, project: project, branch: branch, scheduleOverride: nil, xcodeServer: xcodeServer, completion: { (bot, error) -> Void in
                ...
                if let error = error {
                    ...
                    UIUtils.showAlertWithText(text, style: nil, completion: { (_) -> Void in
                        self.dismiss(nil)
                    })
