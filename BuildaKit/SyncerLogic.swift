...
extension StandardSyncer {
    ...
    internal func syncRepoWithName(repoName: String, completion: @escaping () -> ()) {
        
        self._sourceServer.getRepo(repo: repoName, completion: { (repo, error) -> () in
    ...
    private func syncRepoWithNameAndMetadata(repoName: String, repo: RepoType, completion: @escaping () -> ()) {
        
        //pull PRs from source server
        self._sourceServer.getOpenPullRequests(repo: repoName, completion: { (prs, error) -> () in
    ...
    private func syncRepoWithPRs(repoName: String, repo: RepoType, prs: [PullRequestType], completion: @escaping () -> ()) {
        ...
        if self._watchedBranchNames.count > 0 {
            ...
            self._sourceServer.getBranchesOfRepo(repo: repoName, completion: { (branches, error) -> () in
    ...
    private func syncRepoWithPRsAndBranches(repoName: String, repo: RepoType, prs: [PullRequestType], branches: [BranchType], completion: @escaping () -> ()) {
    ...
    public func syncPRsAndBranchesAndBots(repo: RepoType, repoName: String, prs: [PullRequestType], branches: [BranchType], bots: [Bot], completion: @escaping () -> ()) {
        ...
        -> BotActions {
            ...
            for pr in prs {
                ...
                if let bot = mappedBots[botName] {
                    ...
                    let _ = mappedBots.removeValue(forKey: botName)
                } else {
...
//            self.watchedBranchNames.value = foundBranchesToWatch
    ...
    private func applyResolvedSyncPairs(syncPairs: [SyncPair], completion: @escaping () -> ()) {
        
        //actually kick the sync pairs off
        let group = DispatchGroup()
        for i in syncPairs {
            group.enter()
            i.start(completion: { (error) -> () in
                if let error = error {
