...
public class NetworkUtils {
    public typealias VerificationCompletion = (_ success: Bool, _ error: Error?) -> Void
    ...
    public class func checkAvailabilityOfServiceWithProject(project: Project, completion: @escaping VerificationCompletion) {
        self.checkService(project: project, completion: { success, error in
            ...
            self.checkValidityOfSSHKeys(blueprint: credentialValidationBlueprint, completion: { (success, error) -> Void in
    ...
    private class func checkService(project: Project, completion: @escaping VerificationCompletion) {
        let auth = project.config.serverAuthentication
        ...
        //we have a repo name
        server.getRepo(repo: repoName, completion: { (repo, error) -> Void in
    ...
    public class func checkAvailabilityOfXcodeServerWithCurrentSettings(config: XcodeServerConfig, completion: @escaping (_ success: Bool, _ error: Error?) -> Void) {
        ...
        xcodeServer.logout { (_, error) -> Void in
            ...
            xcodeServer.getUserCanCreateBots({ (canCreateBots, error) -> Void in
    ...
    class func checkValidityOfSSHKeys(blueprint: SourceControlBlueprint, completion: (_ success: Bool, _ error: Error?) -> Void) {
